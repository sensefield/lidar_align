#!/usr/bin/env python3
"""Apply a rigid-body offset to Odometry messages in a ROS2 bag.
...
Usage:
  inject_offset_odom.py <input_bag> <output_bag>
    --dx DX --dy DY --dz DZ
...
def apply_delta_to_odom(msg: Odometry, delta_t: tuple, delta_q: tuple) -> Odometry:
    ...
    new_msg.pose.pose.position.x = p.x + delta_t_world[0]
    new_msg.pose.pose.position.y = p.y + delta_t_world[1]
    new_msg.pose.pose.position.z = p.z + delta_t_world[2]
    norm = math.sqrt(sum(v**2 for v in q_new))
    new_msg.pose.pose.orientation.w = q_new[0] / norm
    new_msg.pose.pose.orientation.x = q_new[1] / norm
    new_msg.pose.pose.orientation.y = q_new[2] / norm
    new_msg.pose.pose.orientation.z = q_new[3] / norm
...
def inject(input_bag: str, output_bag: str, odom_topic: str,
    ...
    for t in topic_types:
        if t.name == odom_topic:
            odom_type = t.type

    if odom_type is None:
        print(f"[WARN] '{odom_topic}' not found. Topics: {[t.name for t in topic_types]}",
              file=sys.stderr)
    ...
    while reader.has_next():
        ...
        if topic == odom_topic and odom_type == "nav_msgs/msg/Odometry":
            msg = deserialize_message(data, Odometry)
            msg = apply_delta_to_odom(msg, delta_t, delta_q)
            data = serialize_message(msg)
            n_modified += 1
        writer.write(topic, data, timestamp)

    print(f"Modified {n_modified} Odometry messages on '{odom_topic}'.", file=sys.stderr)
...
def main():
    ...
    p.add_argument("--odom-topic", default="/localization/kinematic_state")
    args = p.parse_args()
    ...
    inject(
        args.input_bag, args.output_bag, args.odom_topic,
